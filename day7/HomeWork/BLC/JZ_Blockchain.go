...
//1.创建创世区块
func JZ_CreateBlockchainWithGensisBlock(address string, nodeID string) *JZ_Blockchain {
...
//4.获取Blockchain对象
func JZ_GetBlockchain(nodeID string) *JZ_Blockchain {
...
// 获取区块链最大高度
func (blc *JZ_Blockchain) JZ_GetBestHeight() int64 {

	block := blc.JZ_Iterator().JZ_Next()

	return block.JZ_Height
}

// 获取区块所有哈希
func (blc *JZ_Blockchain) JZ_GetBlockHashes() [][]byte {

	blockIterator := blc.JZ_Iterator()

	var blockHashs [][]byte

	for {

		block := blockIterator.JZ_Next()
		blockHashs = append(blockHashs, block.JZ_Hash)
...
// 获取对应哈希的区块
func (blc *JZ_Blockchain) JZ_GetBlock(bHash []byte) ([]byte, error)  {
...
// 将同步请求的主链区块添加到区块链
func (blc *JZ_Blockchain) JZ_AddBlock(block *JZ_Block) error {
	...
	err = blc.JZ_DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			blockExist := b.Get(block.JZ_Hash)
			if blockExist != nil {
			...
			blockInDB := JZ_DeSerializeBlock(b.Get(blockHash))

			if blockInDB.JZ_Height < block.JZ_Height {
