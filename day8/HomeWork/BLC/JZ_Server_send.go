...
func JZ_sendTx(toAddress string, tx *JZ_Transaction)  {

	data := JZ_TxData{nodeAddress, tx.JZ_Serialize()}
	payload := JZ_gobEncode(data)
	request := append(JZ_commandToBytes(COMMAND_TX), payload...)
